...
isthisspace(MMIOT *f, int i)
{
    ...
    if ( c & 0x80 )
	return 0;
    return isspace((unsigned char)c) || ((unsigned char)c < ' ');
}
...
puturl(char *s, int size, MMIOT *f, int display)
{
	...
	if ( c == '\\' && size-- > 0 ) {
	    ...
	    if ( !( ispunct(c) || isspace((unsigned char)c) ) )
		Qchar('\\', f);
	}
	...
	if ( c == '&' )
	    ...
	    Qstring("%22", f);
	else if ( isalnum(c) || ispunct(c) || (display && isspace((unsigned char)c)) )
	    Qchar(c, f);
...
eatspace(MMIOT *f)
{
    ...
    for ( ; ((c=peek(f, 1)) != EOF) && isspace((unsigned char)c); pull(f) )
	;
    return c;
}
...
linkysize(MMIOT *f, Footnote *ref)
{
    ...
    if ( isspace((unsigned char)peek(f,0)) ) {
	pull(f);	/* eat '=' */
...
maybe_tag_or_link(MMIOT *f)
{
	...
	else if ( !isspace((unsigned char)c) && process_possible_link(f, size) ) {
	    shift(f, size+1);
	    return 1;
	}
    }
...
maybe_autolink(MMIOT *f)
{
	...
	if ( c == '\\' ) {
	    ...
	    continue;
	else if ( isspace((unsigned char)c) || strchr("'\"()[]{}<>`", c) || c == MKD_EOLN )
	    break;
    }
...
text(MMIOT *f)
{
	...
	case '^':   if ( is_flag_set(&f->flags, MKD_NOSUPERSCRIPT)
			    || is_flag_set(&f->flags, MKD_TAGTEXT)
			    || (f->last == 0)
			    || ((ispunct(f->last) || isspace((unsigned char)f->last))
						    && f->last != ')')
			    || isthisspace(f,1) )
			Qchar(c,f);
		    else {
...
printtable(Paragraph *pp, MMIOT *f)
{
	...
	for (end=start ; (end < S(dash->text)) && p[end] != '|'; ++ end ) {
	    if ( p[end] == '\\' )
		++ end;
	    else if ( !isspace((unsigned char)p[end]) ) {
