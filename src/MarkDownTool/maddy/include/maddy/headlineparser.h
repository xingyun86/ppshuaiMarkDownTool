...
public:
  ...
  IsStartingLine(const std::string& line)
  {
    ...
    static std::regex reNoSpace("^(?:#){1,6}(.*)");
    return std::regex_match(line, re) || std::regex_match(line, reNoSpace);
  }
...
protected:
  ...
  parseBlock(std::string& line) override
  {
    ...
    static std::vector<std::regex> hlRegexNoSpace = {
    std::regex("^#(.*)")
    , std::regex("^(?:#){2}(.*)")
    , std::regex("^(?:#){3}(.*)")
    , std::regex("^(?:#){4}(.*)")
    ...
    for (auto i = 5; i >= 0; --i)
    {
      ...
      line = std::regex_replace(line, hlRegexNoSpace[i], hlReplacement[i]);
    }
  }
